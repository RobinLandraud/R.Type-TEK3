...
class Clock
{
  public:
     ...
     * \brief Check if the clock is ticking
  ...
  private:
    ...
    std::chrono::duration<float> deltaTime; /*!< Time between the last loop and
                                               the current loop */
    std::chrono::time_point<std::chrono::steady_clock> begin; /*!< Time of the
                                                                 last loop */
    std::chrono::time_point<std::chrono::steady_clock> now;   /*!< Time of the
                                                                 current loop */
};
...
class ClocksComponent
{
  public:
    ...
    ClocksComponent& operator=(
        ClocksComponent const&) = default; /*!< Default copy assignment operator
                                            */
    ClocksComponent& operator=(
        ClocksComponent&&) noexcept = default; /*!< Default move assignment
                                                  operator */

    /**
     * \fn Clock& getClock(std::string const& name)
     */
    ...
    /**
     * \fn void addClock(std::string const& name, Clock const& clock)
     * \brief Add a clock
  ...
  private:
    Clock defaultClock; /*!< Default clock */
